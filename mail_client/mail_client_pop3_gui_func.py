...
class TabAll:
    ...
    def on_message_selected(self, event):
        ...
        if selected_item != ():
            ...
            self.content_label.config(text=parse_message(mail.message_as_string))
            if mail.read == False:
                mail.read = True
...
class TabBySender:
    ...
    def on_message_selected(self, event):
        selected_item = event.widget.curselection()
        if selected_item != ():
            mail : MMailMessage = self.mails[self.message_listbox.get(selected_item[0])]
            self.content_label.config(text=parse_message(mail.message_as_string))
